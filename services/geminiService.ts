...
import { calculateFibonacciRetracements, calculateFibonacciExtensions, isUptrendImpulse } from "../utils/fibonacci";
...
export const analyzeChartWithGemini = async (
  ...
  try {
    ...
    // Post-procesar Fibonacci: recalcular los niveles automáticamente basado en los puntos A, B, C
    if (parsedData.analisis_fibonacci) {
      const fibonacci = parsedData.analisis_fibonacci;

      // Verificar que tenemos los puntos necesarios
      if (fibonacci.precio_inicio_impulso && fibonacci.precio_fin_impulso) {
        const pointA = fibonacci.precio_inicio_impulso;
        const pointB = fibonacci.precio_fin_impulso;
        const isUptrend = isUptrendImpulse(pointA, pointB);

        // Recalcular niveles de retroceso automáticamente
        fibonacci.niveles_retroceso = calculateFibonacciRetracements(pointA, pointB, isUptrend);

        // Si hay punto C, recalcular extensiones
        if (fibonacci.precio_fin_retroceso) {
          const pointC = fibonacci.precio_fin_retroceso;
          fibonacci.niveles_extension = calculateFibonacciExtensions(pointA, pointB, pointC, isUptrend);
        }

        console.log("✅ Fibonacci recalculado automáticamente:", {
          impulso: isUptrend ? 'Alcista' : 'Bajista',
          pointA,
          pointB,
          pointC: fibonacci.precio_fin_retroceso,
          retrocesos: fibonacci.niveles_retroceso?.length,
          extensiones: fibonacci.niveles_extension?.length
        });
      }
    }
